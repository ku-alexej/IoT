...
prepare_manifest() {
    ...
    cp "${DIR_SCRIPT}/../confs/01_deployment_v2.yaml" "${DIR_REPO}/deployment.yaml"

    git add deployment.yaml
    if ! git diff --cached --quiet; then
        log_warning "manifest" "committing changes"
        git commit -m "chore: update deployment manifest (v2)" >/dev/null 2>&1
...
title "Preparing deployment manifest (v2)"
